-- B-10b probe: R1 happy D1 (esperado: SUCCESS stock 13→10 inv 13→10 wac 5 kardex devolution_reverse uc5 ref=D1 audit U1)
...
BEGIN
  ...
  WHERE d.id = 'deafe436-ab12-404c-9975-b519623a8da9';

  BEGIN
    v_res := public.reverse_devolution('deafe436-ab12-404c-9975-b519623a8da9', 'R1 happy D1', NULL);
  ...
  SELECT jsonb_build_object(
    ...
    'movs_dev', (SELECT count(*) FROM public.stock_movements WHERE reference_id = 'deafe436-ab12-404c-9975-b519623a8da9'),
    ...
    'audits_dev', (SELECT count(*) FROM public.audit_logs WHERE action='REVERSE_DEVOLUTION' AND record_id='deafe436-ab12-404c-9975-b519623a8da9'),
    'audit_last', (SELECT jsonb_build_object('user_id',a.user_id,'store',a.store_id,'meta',a.metadata)
                 FROM public.audit_logs a WHERE a.action='REVERSE_DEVOLUTION' AND a.record_id='deafe436-ab12-404c-9975-b519623a8da9'
  ...
  INSERT INTO b10b_probe VALUES ('result', jsonb_build_object(
    'probe', 'R1 happy D1', 'expected', 'SUCCESS stock 13→10 inv 13→10 wac 5 kardex devolution_reverse uc5 ref=D1 audit U1',
