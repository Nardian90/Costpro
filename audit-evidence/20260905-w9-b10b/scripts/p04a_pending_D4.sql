-- B-10b probe: R4a pending D4 (esperado: ERR_INVALID_STATUS)
...
BEGIN
  ...
  WHERE d.id = 'b10b0000-0000-4000-8000-0000000003a4';

  BEGIN
    v_res := public.reverse_devolution('b10b0000-0000-4000-8000-0000000003a4', 'R4a pending D4', NULL);
  ...
  SELECT jsonb_build_object(
    ...
    'movs_dev', (SELECT count(*) FROM public.stock_movements WHERE reference_id = 'b10b0000-0000-4000-8000-0000000003a4'),
    ...
    'audits_dev', (SELECT count(*) FROM public.audit_logs WHERE action='REVERSE_DEVOLUTION' AND record_id='b10b0000-0000-4000-8000-0000000003a4'),
    'audit_last', (SELECT jsonb_build_object('user_id',a.user_id,'store',a.store_id,'meta',a.metadata)
                 FROM public.audit_logs a WHERE a.action='REVERSE_DEVOLUTION' AND a.record_id='b10b0000-0000-4000-8000-0000000003a4'
  ...
  INSERT INTO b10b_probe VALUES ('result', jsonb_build_object(
    'probe', 'R4a pending D4', 'expected', 'ERR_INVALID_STATUS',
