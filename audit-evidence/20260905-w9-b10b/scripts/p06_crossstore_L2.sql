-- B-10b probe: R6 cross-store U2→L2 (esperado: ERR_UNAUTHORIZED)
SELECT set_config('request.jwt.claims', '{"role":"authenticated","sub":"b10b0000-0000-4000-8000-0000000001b2"}', true);
...
BEGIN
  PERFORM set_config('request.jwt.claims', '{"role":"authenticated","sub":"b10b0000-0000-4000-8000-0000000001b2"}', true);
  ...
  BEGIN
    v_res := public.reverse_devolution('b10b0000-0000-4000-8000-0000000003b2', 'R6 cross-store U2→L2', NULL);
  ...
  INSERT INTO b10b_probe VALUES ('result', jsonb_build_object(
    'probe', 'R6 cross-store U2→L2', 'expected', 'ERR_UNAUTHORIZED',
