-- B-10b probe: R7 forged p_user_id=GA, claims U1 → L1 (esperado: SUCCESS como U1 (auth.uid()); audit.user_id=U1 ≠ GA; uc=4.5)
...
BEGIN
  ...
  WHERE d.id = 'b10b0000-0000-4000-8000-0000000003b1';

  BEGIN
    v_res := public.reverse_devolution('b10b0000-0000-4000-8000-0000000003b1', 'R7 forged p_user_id=GA, claims U1 → L1', 'b10b0000-0000-4000-8000-0000000001c3');
  ...
  SELECT jsonb_build_object(
    ...
    'movs_dev', (SELECT count(*) FROM public.stock_movements WHERE reference_id = 'b10b0000-0000-4000-8000-0000000003b1'),
    ...
    'audits_dev', (SELECT count(*) FROM public.audit_logs WHERE action='REVERSE_DEVOLUTION' AND record_id='b10b0000-0000-4000-8000-0000000003b1'),
    'audit_last', (SELECT jsonb_build_object('user_id',a.user_id,'store',a.store_id,'meta',a.metadata)
                 FROM public.audit_logs a WHERE a.action='REVERSE_DEVOLUTION' AND a.record_id='b10b0000-0000-4000-8000-0000000003b1'
  ...
  INSERT INTO b10b_probe VALUES ('result', jsonb_build_object(
    'probe', 'R7 forged p_user_id=GA, claims U1 → L1', 'expected', 'SUCCESS como U1 (auth.uid()); audit.user_id=U1 ≠ GA; uc=4.5',
