...
INSERT INTO b8_cl VALUES('TXH_POST_STATE', (
  ...
  FROM public.transactions t WHERE t.id='b8cd0000-0000-4000-8000-00000000d001'));
...
DELETE FROM public.business_events WHERE entity_id::text LIKE 'b8cc0000%';
DELETE FROM public.audit_logs WHERE store_id::text LIKE 'b8ca0000%';
DELETE FROM public.user_audit_log WHERE target_user_id::text LIKE 'b8cb0000%';
DELETE FROM public.payment_transactions WHERE store_id::text LIKE 'b8ca0000%';
DELETE FROM public.transaction_items WHERE transaction_id IN (SELECT id FROM public.transactions WHERE store_id::text LIKE 'b8ca0000%');
DELETE FROM public.transactions WHERE store_id::text LIKE 'b8ca0000%';
DO $$
BEGIN
  IF to_regclass('public.kardex_entries') IS NOT NULL THEN
    DELETE FROM public.kardex_entries WHERE product_id::text LIKE 'b8cc0000%';
  END IF;
END $$;
DELETE FROM public.stock_movements WHERE store_id::text LIKE 'b8ca0000%';
DELETE FROM public.inventory WHERE store_id::text LIKE 'b8ca0000%';
DELETE FROM public.products WHERE store_id::text LIKE 'b8ca0000%';
DELETE FROM public.user_store_memberships WHERE store_id::text LIKE 'b8ca0000%';
...
-- ESTA misma request atómica (ventana de milisegundos, scope LIKE b8cb0000%).
ALTER TABLE public.profiles DISABLE TRIGGER prevent_hard_delete_profile;
DELETE FROM public.profiles WHERE id::text LIKE 'b8cb0000%';
DELETE FROM auth.users WHERE id::text LIKE 'b8cb0000%';
...
INSERT INTO b8_cl VALUES('GUARD_RESTORED', jsonb_build_object(
  ...
  'profiles_fixture', (SELECT count(*) FROM public.profiles WHERE id::text LIKE 'b8cb0000%'),
  'auth_users_fixture', (SELECT count(*) FROM auth.users WHERE id::text LIKE 'b8cb0000%')));
...
DELETE FROM public.stores WHERE id::text LIKE 'b8ca0000%';
...
INSERT INTO b8_cl VALUES('RESIDUE_CHECK', jsonb_build_object(
  'stores',            (SELECT count(*) FROM public.stores WHERE id::text LIKE 'b8ca0000%'),
  'profiles',          (SELECT count(*) FROM public.profiles WHERE id::text LIKE 'b8cb0000%'),
  'auth_users',        (SELECT count(*) FROM auth.users WHERE id::text LIKE 'b8cb0000%'),
  'memberships',       (SELECT count(*) FROM public.user_store_memberships WHERE store_id::text LIKE 'b8ca0000%'),
  'products',          (SELECT count(*) FROM public.products WHERE id::text LIKE 'b8cc0000%'),
  'inventory',         (SELECT count(*) FROM public.inventory WHERE store_id::text LIKE 'b8ca0000%'),
  'transactions',      (SELECT count(*) FROM public.transactions WHERE id::text LIKE 'b8cd0000%'),
  'transaction_items', (SELECT count(*) FROM public.transaction_items WHERE transaction_id::text LIKE 'b8cd0000%'),
  'payments',          (SELECT count(*) FROM public.payment_transactions WHERE id::text LIKE 'b8cd0000%' OR store_id::text LIKE 'b8ca0000%'),
  'stock_movements',   (SELECT count(*) FROM public.stock_movements WHERE store_id::text LIKE 'b8ca0000%'),
  'audit_logs',        (SELECT count(*) FROM public.audit_logs WHERE store_id::text LIKE 'b8ca0000%'),
  'user_audit_log',    (SELECT count(*) FROM public.user_audit_log WHERE target_user_id::text LIKE 'b8cb0000%'),
  'business_events',   (SELECT count(*) FROM public.business_events WHERE entity_id::text LIKE 'b8cc0000%'),
  'kardex_entries',    (CASE WHEN to_regclass('public.kardex_entries') IS NOT NULL THEN (SELECT count(*)::text FROM public.kardex_entries WHERE product_id::text LIKE 'b8cc0000%') ELSE 'n/a' END)
));
