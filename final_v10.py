...
def fix():
    ...
    component_match = re.search(r'const CostSheetAnnexEditor: React\.FC<CostSheetAnnexEditorProps> = React\.memo\(.*?\{', content, re.DOTALL)
    if component_match:
        insertion_pos = component_match.end()
        extra_logic = """
  ...
  const handleSolve = () => {
    ...
    setTimeout(() => {
      try {
        ...
        content = content[:insertion_pos] + extra_logic + content[insertion_pos:]

    # 3. Completely replace the first card (the header)
    # The header starts with <div className="flex flex-col lg:flex-row... and ends before <ProductInventoryPicker

    header_pattern = r'<div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-card/50 p-6 rounded-\[2\.5rem\] border border-border/50 shadow-xl backdrop-blur-md">.*?Añadir Fila.*?</Button>\s+</div>\s+</div>'

    new_header = """<div className="flex flex-col gap-6 bg-card/50 p-8 rounded-[2.5rem] border border-border/50 shadow-xl backdrop-blur-md">
          {/* Action Buttons Row */}
    ...
    # We use a trick to escape the backslashes in the replacement string
