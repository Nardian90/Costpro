...
def fix():
    ...
    # Add state & handleSolve
    state_anchor = "const [targetRowIndex, setTargetRowIndex] = React.useState<number | null>(null);"
    extra_logic = """
...
"""
    logic_part = logic_part.replace(state_anchor, state_anchor + extra_logic)

    # 2. Extract Table and Footer (the part after the original header)
    # The original header ends with a </div> at line 209 (relative to return start)
    # Let's find ProductInventoryPicker and everything after it
    picker_pos = content.find('<ProductInventoryPicker')
    tail_part = content[picker_pos:]

    # 3. Construct the NEW return statement
    # We'll use the layout I designed

    action_buttons = """          {/* Action Buttons Row */}
    ...
    target_price_ui = """                <div className="flex items-center gap-3 bg-background/50 px-4 py-1.5 rounded-xl border border-primary/10 ml-2">
