...
# 1. Definir las descripciones completas en español incluyendo AUTO_SUPPLY
new_descriptions = """const RULE_DESCRIPTIONS: Record<string, any> = {
    ...
    "CASH_FILL": {
        ...
        "errors": [
            ...
            "Sin productos: Si no hay productos activos cuyo precio sea superior al remante."
        ]
    },
    "WILDCARDS": {
...
# Reemplazar el objeto RULE_DESCRIPTIONS
pattern_desc = re.compile(r'const RULE_DESCRIPTIONS: Record<string, any> = \{.*? \};', re.DOTALL)
content = pattern_desc.sub(new_descriptions, content)

# 2. Traducir etiquetas de títulos de reglas
...
pattern_label = re.compile(r'\{\s*rule\.tipo\.replace\(/_/g,\s*\' \'\)\s*\}')
content = pattern_label.sub(translation_map_code, content)
