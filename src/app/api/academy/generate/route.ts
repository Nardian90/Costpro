...
export async function POST(req: NextRequest) {
  try {
    ...
    const apiKey = aiApiKey || process.env.NEXT_PUBLIC_GEMINI_API_KEY;

    if (!apiKey) {
       return NextResponse.json({ error: 'No API key provided for Gemini AI. Please configure it in your profile or system settings.' }, { status: 400 });
    }
