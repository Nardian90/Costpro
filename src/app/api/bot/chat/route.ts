  ...
  try {
    ...
    try {
      ...
      return NextResponse.json({
        ...
        details: (process.env.NODE_ENV !== 'production' || !!process.env.VITEST) ? errorMsg : undefined
      }, { status: 502 });
    }
    ...
    return NextResponse.json({
      error: 'Error interno',
      details: (process.env.NODE_ENV !== 'production' || !!process.env.VITEST) ? msg : 'Error interno del servidor'
    }, { status: 500 });
  }
}
