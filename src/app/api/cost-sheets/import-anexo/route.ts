...
const handler = withAuth(async (req, session) => {
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'test' ? error.message : 'Error interno del servidor' }, { status: 500 });
  }
