...
const handler = withAuth(async (req, session) => {
  ...
  try {
    ...
    for (const op of batch.operations) {
      ...
      try {
        ...
        await supabase.from('sync_log').upsert({
          ...
          response_data: { error: process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'test' ? err.message : 'Error de sincronización' },
          store_id: op.payload.p_store_id || null,
        });
        ...
        results.push({
          ...
          status,
          // FIX-SEC-022: Hide internal error details in sync responses
          error: process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'test' ? err.message : 'Error de sincronización',
          serverData,
        });
      }
    }
    ...
    return NextResponse.json({ error: process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'test' ? err.message : 'Error interno del servidor' }, { status: 500 });
  }
});
