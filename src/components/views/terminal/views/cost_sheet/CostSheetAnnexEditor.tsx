...
// P3-2: AlertDialog eliminado — reemplazado por <ConfirmDialog> unificado.
      ...
      <PopoverContent className="w-[300px] p-0 z-[60]" align="start" sideOffset={4}>
        <Command className="rounded-lg border-border/60">
          ...
          <CommandList className="max-h-[220px]">
            <CommandEmpty className="text-xs py-2 text-muted-foreground/70">
              Sin resultados
            </CommandEmpty>
            <CommandGroup>
              {options.map((opt) => {
                ...
                return (
                  <CommandItem
                    ...
                    className={cn(
                      "flex items-center gap-2 px-2 py-1.5 text-xs cursor-pointer rounded-sm",
                      isSelected && "bg-primary/10 text-primary"
                    )}
                  >
                    <span className="font-mono font-bold text-xs text-muted-foreground/70 w-10 shrink-0 tabular-nums">
                      {opt.id}
                    </span>
...
const CostSheetAnnexEditor: React.FC<CostSheetAnnexEditorProps> = React.memo(({
  ...
  return (
    <div className="space-y-0 animate-in fade-in slide-in-from-bottom-4 duration-500">
       ...
       <div className="flex flex-wrap items-center gap-1.5 px-2 py-1.5 bg-muted/30 border-b border-border/20">
          <div className="relative group">
             ...
             <Button
               ...
               size="sm"
               asChild
               className="h-9 px-2 text-xs font-bold uppercase tracking-wider text-muted-foreground hover:text-primary hover:bg-primary/5 gap-1"
             >
          ...
          <Button
             ...
             className="h-9 px-2.5 text-xs font-bold uppercase tracking-wider text-muted-foreground hover:text-primary hover:bg-primary/5 gap-1"
          >
          ...
          {isCoefficientAnnex && (
            <div className="flex items-center gap-2 ml-auto">
              <span className="text-xs font-bold uppercase tracking-wider text-primary/70">Coef:</span>
              <input
                  type="text"
                  value={localCoef}
                  ...
                  className="w-16 h-9 px-1 bg-background border border-primary/20 text-xs font-black font-mono text-primary focus:outline-none focus:ring-1 focus:ring-primary/30 transition-all text-center rounded"
              />
              <Select
              ...
              >
                  <SelectTrigger className="h-9 min-w-[110px] bg-background border-primary/20 rounded text-xs font-bold uppercase hover:border-primary/40 transition-colors">
                      <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="rounded border-primary/20">
                      {annex.id === 'I' ? (
                        <>
                          <SelectItem value="NORMA DE CONSUMO" className="text-xs font-bold uppercase">Norma Consumo</SelectItem>
                          <SelectItem value="PRECIO UNITARIO" className="text-xs font-bold uppercase">Precio Unit.</SelectItem>
                          <SelectItem value="AMBOS" className="text-xs font-bold uppercase">Ambos</SelectItem>
                        </>
                      ) : annex.id === 'II' ? (
                        <>
                          <SelectItem value="HORAS MENSUALES" className="text-xs font-bold uppercase">Horas Mens.</SelectItem>
                          <SelectItem value="TARIFA $/H" className="text-xs font-bold uppercase">Tarifa $/h</SelectItem>
                          <SelectItem value="CANT. OBREROS" className="text-xs font-bold uppercase">Cant. Obreros</SelectItem>
                        </>
                      ) : null}
                  </SelectContent>
              </Select>
              <div className="flex items-center gap-1.5 bg-background/50 px-2 py-0.5 rounded border border-primary/10">
                  <span className="text-xs font-bold uppercase tracking-wider text-primary/70">Auto</span>
                  <Switch
              ...
              {showTargetPriceInput && (
                <>
                  <span className="text-xs font-bold uppercase tracking-wider text-primary/70 ml-2">Target:</span>
                  <input
                      disabled={isSolving}
                      type="number"
                      value={targetPrice}
                      ...
                      className="w-20 h-9 px-1 rounded bg-background border border-primary/40 text-xs font-black font-mono text-primary focus:outline-none focus:ring-1 focus:ring-primary/30 transition-all text-center placeholder:text-primary/70"
                  />
                  <Button
                      size="sm"
                      onClick={handleSolve}
                      disabled={isSolving || !targetPrice}
                      className="h-9 px-2 rounded text-xs font-bold uppercase tracking-wider bg-primary text-foreground hover:bg-primary/90 active:scale-95 transition-all gap-1"
                  >
                      {isSolving ? (
         ...
         <div className="flex items-center gap-2 px-2 py-1 bg-warning/5 border-b border-border/20">
            ...
            {referencingSections.map((ref, idx) => (
              <button
                ...
                className="text-xs font-bold text-warning hover:text-warning/80 hover:bg-warning/10 px-1.5 py-0.5 rounded transition-colors truncate max-w-[200px]"
            ...
            <Table className={cn(layoutMode === 'grid' && "hidden sm:table")}>
                <TableHeader className="sticky top-0 z-20">
                    <TableRow className="bg-muted/80 hover:bg-transparent border-b border-border/40 h-7">
                        <TableHead className="w-[32px] px-1 py-0 text-center text-xs font-black tracking-widest text-muted-foreground/70 border-r border-border/20">
                          #
                        </TableHead>
                        {annex.columns.map((col: CostSheetColumn) => {
                            ...
                            let colClass = 'px-1.5 py-0 text-left text-xs font-black tracking-widest text-muted-foreground/70 border-r border-border/20';
                            
                            if (col.key === 'no') {
                              colClass = 'w-[56px] px-1.5 py-0 text-center text-xs font-black tracking-widest text-muted-foreground/70 border-r border-border/20';
                              ...
                              colClass = 'w-[120px] px-2 py-0 text-right text-xs font-black tracking-widest text-muted-foreground/70 border-r border-border/20';
                              ...
                              colClass = 'w-[90px] px-2 py-0 text-center text-xs font-black tracking-widest text-muted-foreground/70 border-r border-border/20';
                            } else if (colKey.includes('um') || colKey === 'um') {
                              colClass = 'w-[50px] px-1.5 py-0 text-center text-xs font-black tracking-widest text-muted-foreground/70 border-r border-border/20';
                            } else if (colKey.includes('code') || colKey.includes('código') || colKey.includes('clasif')) {
                              colClass = 'w-[80px] px-2 py-0 text-center text-xs font-black tracking-widest text-muted-foreground/70 border-r border-border/20';
                              ...
                              colClass = 'px-2 py-0 text-left text-xs font-black tracking-widest text-muted-foreground/70 border-r border-border/20 min-w-[180px]';
                            }
                        ...
                        <TableHead className="px-1 py-0 text-center text-xs font-black tracking-widest text-muted-foreground/70 w-[48px]">
                        </TableHead>
                    </TableRow>
                </TableHeader>
                <TableBody>
                    {annex.data.map((row: Record<string, string | number | boolean | undefined>, rowIndex: number) => {
                        ...
                        <TableRow key={rowIndex} className={cn(
                          ...
                          rowIndex % 2 === 0 && 'bg-muted/[0.15]'
                            ...
                            {/* Row number column */}
                            <TableCell className="w-[56px] px-1.5 py-0 text-center text-xs font-mono text-muted-foreground/70 tabular-nums border-r border-border/15">
                              {rowIndex + 1}
                            </TableCell>
                            {annex.columns.map((col: CostSheetColumn) => {
                                    ...
                                    {col.formula ? (
                                        <div className="flex items-center justify-end gap-1">
                                            <span className={cn(
                                              ...
                                              isZero(col.key) ? 'text-muted-foreground/70 font-medium' : 'text-primary font-black'
                                            )}>
                                                {formatCurrency((calculatedAnnex?.data[rowIndex]?.[col.key] ?? 0)).replace('$', '').trim()}
                                            </span>
                                            <FunctionSquare className="w-2 h-2 text-primary/70 shrink-0" />
                                        </div>
                                        ...
                                        <div className="relative group/cell">
                                            <Input
                                                ...
                                                className={cn(
                                                  'h-9 text-xs px-1.5 py-0 bg-transparent border border-transparent hover:border-border/30 focus:border-primary/40 focus:outline-none rounded transition-colors font-medium',
                                                  typeof annex.data[rowIndex][col.key] === 'string' && annex.data[rowIndex][col.key] !== '' && 'text-primary',
                                                  typeof row[col.key] === "number" && isZero(col.key) && 'text-muted-foreground/70 font-medium'
                                                )}
                                                placeholder={(() => {
                                                    const val = annex.data[rowIndex][col.key];
                                                    const coef = annex.coefficient !== undefined ? annex.coefficient : 1;
                                            ...
                                            {typeof annex.data[rowIndex][col.key] === 'string' && annex.data[rowIndex][col.key] !== '' && (
                                                <FunctionSquare className="absolute top-0.5 right-0.5 w-2 h-2 text-primary/70" />
                                            )}
                                        </div>
                                    )}
                                </TableCell>
                  ...
                  <TableRow className="bg-primary/5 hover:bg-primary/5 border-t-2 border-primary/20 h-7">
                    <TableCell colSpan={annex.columns.length + 1} className="px-1.5 py-0">
                      <div className="flex justify-end items-center gap-3">
                        <span className="text-xs text-primary/70 uppercase font-black tracking-[0.15em]">Total {annex.id}</span>
       ...
       {/* P3-2: Import Confirmation Dialog — migrado de AlertDialog a <ConfirmDialog>.
           Unificación P3: mismo componente en todo el módulo COSTOS. */}
       <ConfirmDialog
         open={!!importTarget}
         title="¿Importar anexo desde Excel?"
         message={<>Esta acción reemplazará todos los datos del anexo <strong>"{annex?.title}"</strong> con los datos del archivo seleccionado. Los datos actuales se perderán. ¿Desea continuar?</>}
         variant="destructive"
         ...
         <div className="flex justify-end mt-4 sm:hidden">
            <div className="!p-5 border-primary/20 bg-primary/5 shadow-xl min-w-[240px] w-full rounded-xl">
                ...
                <div className="px-2 py-0.5 rounded-lg bg-warning/10 border border-warning/20 flex items-center gap-1.5 print:hidden">
                            ...
                            <span className="text-xs font-black font-mono text-warning">x{annex.coefficient.toFixed(4)}</span>
                        </div>
                    )}
                </div>
            </div>
