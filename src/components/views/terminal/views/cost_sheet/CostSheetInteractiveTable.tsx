            ...
            <div className="relative">
                {isEditingVH ? (
                    ...
                    <div className="flex items-center justify-end gap-1">
                        ...
                        value={hasChildren
                        ? formatAccounting(safeCalculated.calculated_vh ?? safeCalculated.valor_historico ?? 0)
                        : (row.vh_formula
                            ? formatAccounting(safeCalculated.calculated_vh ?? 0)
                            : (row.hasOwnProperty('valor_historico') ? formatAccounting(row.valor_historico ?? 0) : (row.is_percent ? ((row.value ?? 0) * 100).toFixed(3) : formatAccounting(row.value ?? 0))))}
                        readOnly={true}
                        />
