...
import { Upload, Save, FileSpreadsheet, FileType2, Sparkles, LogOut, ChevronDown } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-56 p-2 rounded-2xl bg-card border-border shadow-2xl">
        ...
        <DropdownMenuItem onClick={onOpenDarian} className="flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer text-primary focus:bg-primary/10">
          <Sparkles className="w-4 h-4" />
          <span className="text-xs font-black uppercase tracking-widest">Asistente DARIAN</span>
        </DropdownMenuItem>
