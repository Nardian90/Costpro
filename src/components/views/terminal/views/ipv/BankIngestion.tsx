...
export function BankIngestion() {
  ...
  const processBankData = async (data: any[]) => {
    ...
    const processingRows = (mappedRows || data).map((r: any) => ({
        ...r,
        fecha: r.date || r.fecha,
        importe_cents: r.amount || r.importe_cents,
        referencia_origen: r.reference || r.transaction_id || r.referencia_origen,
        observaciones: r.description || r.observaciones,
        nombre_cliente: r.customer_name || r.nombre_cliente,
        carnet: r.customer_id || r.carnet
    }));
    let imported = 0;
    let errorsCount = 0;
