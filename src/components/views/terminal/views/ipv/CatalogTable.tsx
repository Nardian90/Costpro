...
export function CatalogTable() {
  ...
  // Default 'with_stock' para ocultar productos agotados (no hacer ruido visual).
  // El usuario puede cambiar a 'all' si quiere ver todo.
  const [stockFilter, setStockFilter] = useState<'all' | 'with_stock' | 'without_stock' | 'negative_stock'>(() => (typeof window !== 'undefined' ? (localStorage.getItem('catalog_stockFilter') as any) || 'with_stock' : 'with_stock'));
...
const handleExportCatalog = async () => {
  ...
  const catalogActions: Action[] = React.useMemo(() => [
    ...
    { id: "classify", label: "Clasificar", icon: Workflow, onClick: handleAutoClassifyHierarchy, variant: "outline", className: "text-primary" },
    ...
    { id: "export-ventas", label: "Exportar Ventas", icon: ShoppingCart, onClick: handleExportSalesTemplate, variant: "outline", className: "text-success" },
    { id: "import-ventas", label: "Importar Ventas", icon: FileSpreadsheet, onClick: () => document.getElementById("sales-import-input")?.click(), variant: "outline", className: "text-warning" },
              ...
              <TableBody>
                  ...
                  {editingId === 'NEW' && (
                      <TableRow className="bg-primary/10">
                          ...
                          <TableCell className="text-right"><div className="flex justify-end gap-1"><Button variant="ghost" size="icon" className="h-8 w-8 text-success" onClick={saveEditing}><Check className="w-4 h-4" /></Button><Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground" onClick={cancelEditing}><X className="w-4 h-4" /></Button></div></TableCell>
                      </TableRow>
                  )}
                  {paginatedResult.map((p) => {
                      ...
                      <TableRow key={p.cod} className={`${isEditing ? "bg-primary/5" : ""} ${isSelected ? "bg-purple-50" : ""}`}>
                                    ...
                                    {p.cod_hijo && (
                                      <Tooltip>
                                        <TooltipTrigger asChild>
                                          <div className="p-0.5 rounded-full bg-blue-100 text-primary cursor-help"><CornerDownRight className="w-3 h-3" /></div>
                                        </TooltipTrigger>
                                        <TooltipContent side="right" className="text-[10px] font-bold uppercase p-2 bg-primary text-foreground border-none shadow-lg">
                          ...
                          <TableCell>{!isEditing && p.priceEffectivenessScore !== undefined && (<div className="flex items-center gap-2"><div className="w-12 h-1.5 bg-muted rounded-full overflow-hidden"><div className={`h-full ${p.priceEffectivenessScore > 70 ? 'bg-success' : p.priceEffectivenessScore > 40 ? 'bg-yellow-500' : 'bg-destructive'}`} style={{ width: `${p.priceEffectivenessScore}%` }} /></div><span className="text-xs font-black">{p.priceEffectivenessScore}</span></div>)}</TableCell>
                          ...
                          <TableCell className="text-right text-xs font-bold text-success">{stats.entradas}</TableCell>
                          <TableCell className="text-right text-xs font-bold text-primary">{stats.salidas}</TableCell>
                          <TableCell className="text-right text-xs font-bold text-warning">{stats.sales}</TableCell>
                          <TableCell className={`text-right text-xs font-black ${stats.final < 0 ? 'text-destructive' : 'text-primary'}`}><div className="flex items-center justify-end gap-1">{stats.final < 0 && <AlertTriangle className="w-3 h-3" />}{stats.final}</div></TableCell>
                          ...
                          <TableCell className="text-center">{isEditing ? (<Switch checked={editForm.isEligibleForCashFill ?? true} onCheckedChange={checked => setEditForm({...editForm, isEligibleForCashFill: checked})} />) : (p.isEligibleForCashFill !== false ? <CheckCircle2 className="w-4 h-4 text-success mx-auto" /> : <span className="opacity-10">-</span>)}</TableCell>
                          ...
                          <TableCell className="text-right"><div className="flex justify-end gap-1">{isEditing ? (<Button variant="ghost" size="icon" className="h-8 w-8 text-success" onClick={saveEditing}><Check className="w-4 h-4" /></Button>) : (<Button variant="ghost" size="icon" className="h-8 w-8 text-primary" onClick={() => startEditing(p)}><Edit2 className="w-4 h-4" /></Button>)}<Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => handleDelete(p.cod)}><Trash2 className="w-4 h-4" /></Button></div></TableCell>
                      </TableRow>
                      );
                  })}
                  </>
    ...
    return (
        <Card className={`p-4 space-y-4 border-none shadow-md bg-card/50 backdrop-blur-sm relative overflow-hidden ${isEditing ? 'ring-2 ring-primary' : ''}`}>
            ...
            <div className="flex justify-between items-start">
                <div><div className="flex items-center gap-2"><p className="text-xs font-black text-primary uppercase tracking-widest">{product.cod}</p>{product.id_grupo && <div className="flex items-center gap-1"><Badge variant="secondary" className="text-[10px] h-4 px-1 opacity-70">G: {product.id_grupo}</Badge>{product.cod_hijo && <Badge variant="outline" className="text-[8px] h-3 px-1 border-blue-200 text-primary font-black"><CornerDownRight className="w-2 h-2 mr-1" /> {product.cod_hijo}</Badge>}</div>}{product.priceEffectivenessScore !== undefined && (<Badge variant="outline" className="text-xs h-3 px-1 border-primary/20 bg-primary/5">Eff: {product.priceEffectivenessScore}%</Badge>)}</div>{isEditing ? (<Input value={editForm.descripcion} onChange={e => setEditForm({...editForm, descripcion: e.target.value})} className="h-8 mt-1 text-xs font-bold" />) : (<h4 className="font-bold text-sm leading-tight">{product.descripcion}</h4>)}</div>
                ...
                <div className="text-center border-l border-border/50"><p className="text-[10px] font-black text-muted-foreground uppercase mb-1">Entrada</p><p className="font-black text-sm text-success">{stats.entradas}</p></div>
                <div className="text-center border-l border-border/50"><p className="text-[10px] font-black text-muted-foreground uppercase mb-1">Salida</p><p className="font-black text-sm text-primary">{stats.salidas}</p></div>
                <div className="text-center border-l border-border/50"><p className="text-[10px] font-black text-muted-foreground uppercase mb-1">Ventas</p><p className="font-black text-sm text-warning">{stats.sales}</p></div>
                <div className="text-center border-l border-border/50"><p className="text-[10px] font-black text-muted-foreground uppercase mb-1">Final</p><p className={`font-black text-sm ${stats.final < 0 ? 'text-destructive' : 'text-primary'}`}>{stats.final}</p></div>
            </div>
