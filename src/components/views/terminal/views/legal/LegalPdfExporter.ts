...
export async function generateLegalPdf(model: any, data: any) {
  ...
  if (model.code === 'SC-3-01') {
    ...
    const drawReceipt = (yOffset: number, copyLabel: string) => {
      ...
      doc.rect(10, yOffset + 10, pageWidth - 20, 25);
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(12);
      doc.text('MODELO ' + model.code, pageWidth - 60, yOffset + 18);
      doc.setFontSize(8);
      doc.setFont('helvetica', 'italic');
      doc.text('Uso Obligatorio - ' + copyLabel, pageWidth - 60, yOffset + 23);

      // Date and Serial No in same row
      doc.setFont('helvetica', 'bold');
      doc.text('FECHA:', 15, yOffset + 32);
      doc.setFont('helvetica', 'normal');
      doc.text(data.fecha_emision || '', 30, yOffset + 32);

      doc.setFont('helvetica', 'bold');
      doc.text('NO. CONSECUTIVO:', pageWidth - 80, yOffset + 32);
      doc.setFont('helvetica', 'normal');
      doc.text(String(data.numero_consecutivo || ''), pageWidth - 45, yOffset + 32);

      // Main Title
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(14);
      doc.text(model.name, pageWidth / 2, yOffset + 45, { align: 'center' });

      // Persona entrega
      doc.setFontSize(10);
      doc.text('RECIBÍ DE:', 15, yOffset + 55);
      doc.setFont('helvetica', 'normal');
      doc.text(String(data.persona_entrega || '').toUpperCase(), 40, yOffset + 55);

      // Table for concepts
      const tableRows = (data.conceptos_tabla || []).map((row: any) => [
        row.concepto?.toUpperCase() || '',
      ...
      doc.text(Number(data.total || 0).toFixed(2), pageWidth - 30, finalY + 10, { align: 'right' });

      doc.setFontSize(9);
      doc.text('CANTIDAD EN LETRAS:', 15, finalY + 18);
      doc.setFont('helvetica', 'normal');
      doc.text(String(data.cantidad_letras || '').toUpperCase(), 15, finalY + 23, { maxWidth: pageWidth - 30 });

      // Signatures
      doc.line(20, finalY + 40, 80, finalY + 40);
      doc.text('FIRMA CAJERO', 35, finalY + 45);

      doc.line(pageWidth - 80, finalY + 40, pageWidth - 20, finalY + 40);
      doc.text('FIRMA ENTREGA', pageWidth - 65, finalY + 45);
    };
