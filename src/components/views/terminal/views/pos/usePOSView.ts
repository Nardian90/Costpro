...
/**
 * usePOSView — Checkout logic hook for POS integration tests.
 * NOT connected to POSView.tsx (that component has its own inline logic).
 * Kept for pos-sale-flow.test.ts integration tests.
 *
 * @deprecated POSView.tsx has its own checkout logic. This hook is maintained
 * only for the integration test suite. Do not add new features here.
 */
  ...
  const handleCheckout = async (
    ...
    try {
      ...
      if (msg.includes('ERR_INSUFFICIENT_STOCK')) {
        ...
        toast.error(msg || "Error en venta", { id: toastId });
      }
    }
  };
