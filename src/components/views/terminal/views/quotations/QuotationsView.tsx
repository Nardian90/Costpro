...
import { Plus, Search, RotateCcw, Loader2, FileText, X, CheckCircle2 } from 'lucide-react';
...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  ...
  accepted: { label: 'Aceptada', color: 'bg-success/10 text-success' },
  ...
  invoiced: { label: 'Facturada', color: 'bg-primary/10 text-primary' },
};
...
export function QuotationsView() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await apiFetch(`/api/quotations?store_id=${storeId}&limit=50`);
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-sm font-bold text-muted-foreground">No hay cotizaciones registradas</p>
        </div>
      ) : (
        <div className="grid gap-2">
          {filtered.map(q => {
            ...
            return (
              <div key={q.id} className="p-4 rounded-xl border border-border bg-card hover:border-primary/30 transition-all">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-mono text-xs font-black text-primary">{q.quotation_number}</span>
                      <span className={cn('text-[10px] font-black uppercase px-1.5 py-0.5 rounded', st.color)}>{st.label}</span>
                    </div>
                    {q.customer_name && <p className="text-sm font-bold truncate">{q.customer_name}</p>}
                    <p className="text-xs text-muted-foreground">{new Date(q.created_at).toLocaleDateString('es-CU')}</p>
                    {q.items && q.items.length > 0 && (
                      <p className="text-xs text-muted-foreground mt-1">{q.items.length} producto(s)</p>
                    )}
                  </div>
                  <div className="text-right shrink-0">
                    <p className="font-mono font-black text-sm">{formatCurrency(Number(q.total_amount))}</p>
                  </div>
                </div>
              </div>
            );
      ...
      {showCreate && <CreateQuotationModal onClose={() => setShowCreate(false)} onCreated={() => { load(); setShowCreate(false); }} storeId={storeId!} />}
    </div>
  );
}

function CreateQuotationModal({ onClose, onCreated, storeId }: { onClose: () => void; onCreated: () => void; storeId: string }) {
  ...
  async function handleSubmit() {
    if (items.length === 0) { toast.error('Al menos 1 producto requerido'); return; }
    setSubmitting(true);
    try {
      await apiFetch('/api/quotations', {
        method: 'POST',
        body: JSON.stringify({ store_id: storeId, items: items.map(i => ({ product_id: i.product_id, quantity: i.quantity, unit_price: i.unit_price })), customer_name: customerName || undefined, customer_phone: customerPhone || undefined }),
      });
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-lg bg-card border border-border rounded-2xl shadow-2xl p-5 space-y-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div>
          <label className="text-xs font-bold uppercase text-muted-foreground">Buscar producto</label>
