...
export default function StockHistoryView() {
  ...
  const { data: movementsData, refetch, isLoading, error } = useStockMovements(user?.activeStoreId, user?.role === 'admin');
  const [searchTerm, setSearchTerm] = useState('');
  const [dateRange, setDateRange] = useState({ from: '', to: '' });

  const onRefresh = () => {
    refetch();
    announce('Historial de stock actualizado', 'polite');
  ...
  const movements = (movementsData || []).filter(mov => {
    ...
    const matchesSearch = !term ||
      (mov.product?.name?.toLowerCase() || '').includes(term) ||
      (mov.product?.sku?.toLowerCase() || '').includes(term) ||
      (mov.reference_doc?.toLowerCase() || '').includes(term);
    const matchesDateFrom = !dateRange.from || ((mov.created_at ?? '') >= dateRange.from);
    const matchesDateTo = !dateRange.to || ((mov.created_at ?? '') <= dateRange.to + 'T23:59:59');
