...
import { Loader2, MessageCircle, Users, Send, Bot, Activity, TrendingUp, AlertCircle } from 'lucide-react';
...
export default function TelegramDashboardView() {
  ...
  const isActive = metrics?.botStatus === 'active';
    ...
    <div className="space-y-4 p-4 max-w-4xl w-full mx-auto overflow-y-auto h-full pb-[env(safe-area-inset-bottom)]">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-black uppercase tracking-tight">Dashboard Telegram</h2>
            <p className="text-xs text-muted-foreground">Métricas en tiempo real</p>
          </div>
        </div>
        <Badge className={cn('text-xs', isActive ? 'bg-blue-500/10 text-blue-600' : 'bg-muted text-muted-foreground')}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3">
        ...
        <Card>
          <CardContent className="p-2.5 sm:p-3">
            ...
            <p className="text-lg sm:text-xl font-black">{metrics?.invitationsToday || 0}</p>
        ...
        <Card>
          <CardContent className="p-2.5 sm:p-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] uppercase font-bold text-muted-foreground">Estado Bot</span>
            </div>
            <p className="text-lg sm:text-xl font-black capitalize">{metrics?.botStatus || '—'}</p>
          </CardContent>
        </Card>
      </div>

      {/* Daily chart */}
      {metrics?.dailyStats && metrics.dailyStats.some(d => d.incoming + d.outgoing > 0) && (
        <Card>
                ...
                return (
                  <div key={day.date} className="flex-1 flex flex-col items-center gap-1">
                    <div className="w-full flex flex-col justify-end" style={{ height: '100px' }}>
                      ...
                      />
                      <div
                        className="w-full bg-cyan-500/70 rounded-b-sm transition-all"
      ...
      <Card>
        <CardContent className="p-4 space-y-3">
          ...
          <p className="text-[10px] text-muted-foreground">
            Prueba el bot sin enviar mensajes reales por Telegram. Usa la configuración GLM de la tienda.
          </p>
          <div className="flex gap-2">
            <Input
              ...
              disabled={testing}
            />
            <Button onClick={handleTestBot} disabled={testing || !testMessage.trim()} size="sm" className="bg-blue-600 hover:bg-blue-700 active:scale-95 text-white min-h-[44px]">
              {testing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Send className="w-3.5 h-3.5" />}
            </Button>
          </div>
          {testResponse && (
            <div className="space-y-2 pt-2 border-t border-border">
              <div className="ml-auto max-w-[75%] rounded-xl px-3 py-2 bg-blue-600 text-white text-xs rounded-br-sm">
                {testResponse.text}
              </div>
      ...
      {metrics?.botStatus === 'not_configured' && (
        <Card>
          <CardContent className="p-4 text-center text-muted-foreground">
            <AlertCircle className="w-8 h-8 opacity-20 mx-auto mb-2" />
            <p className="text-xs">Bot no configurado. Ve a "Bot y Config" para empezar.</p>
          </CardContent>
        </Card>
      )}
    </div>
