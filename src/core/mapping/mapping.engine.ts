...
export class MappingEngine {
  static apply(
    ...
    const mappedData = dataset.map((row, index) => {
      const mappedRow: any = { ...row }; // Mantener original por si acaso
      ...
      for (const rule of activeRules) {
        ...
        if (rawValue !== undefined) {
          try {
           ...
           // Si la columna no existe en esta fila específica pero hay regla
           // Podríamos marcarlo como advertencia o error suave
        }
      }
