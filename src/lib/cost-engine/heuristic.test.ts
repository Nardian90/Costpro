...
describe('Dependency Validation Heuristics', () => {
  ...
  it('should flag A->B->A as CRITICAL cycle', () => {
    ...
    expect(result.validation_errors.some(e => e.type === 'CRITICAL' && e.message.includes('Referencia Circular Detectada'))).toBe(true);
  });
  ...
  it('should flag Parent Ref (Parent->Child->Parent) as WARNING, not CRITICAL', () => {
    ...
    expect(result.validation_errors.some(e => e.type === 'WARNING' && e.message.includes('Validación de Jerarquía'))).toBe(true);
  });
  ...
  it('should flag Sibling references as valid and NOT cycles', () => {
    ...
    const result = validateFicha(ficha);
    expect(result.valid).toBe(true);
    expect(result.validation_errors.filter(e => e.code === 'CYCLE')).toHaveLength(0);
  });
  ...
  it('should flag external section references as INFO', () => {
    ...
    const result = validateFicha(ficha);
    expect(result.valid).toBe(true);
    expect(result.validation_errors.some(e => e.type === 'INFO' && e.message.includes('Vínculo Externo'))).toBe(true);
    expect(result.validation_errors.find(e => e.message.includes('Vínculo Externo'))?.message).toContain('Sección 1');
  });
});
