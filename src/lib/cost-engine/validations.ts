...
export const calculateCostSheetHealth = (
    ...
    // 2. Utility / Cost Validation (Res 148 - 30% cap)
        ...
        if (costVal > 0) {
            ...
            if (ratio > 0.3) {
                results.push({
                    ...
                    message: 'La relación utilidad/costo (' + (ratio * 100).toFixed(2) + '%) supera el límite del 30% establecido en la metodología.',
                    value: ratio,
                    rowId: utilId
                });
            } else {
                results.push({
                    ...
                    message: 'La relación utilidad/costo (' + (ratio * 100).toFixed(2) + '%) está dentro del rango permitido.',
                    value: ratio,
                    rowId: utilId
                });
            }
    ...
    if (s2 > 0) {
        ...
        const limit = 1.5; // Default limit for production
        ...
        if (coef > limit) {
            results.push({
                ...
                message: 'El coeficiente (' + coef.toFixed(2) + ') supera el máximo permitido de ' + limit.toFixed(2) + '.',
                value: coef
            });
        } else {
            results.push({
                ...
                message: 'El coeficiente (' + coef.toFixed(2) + ') está dentro de los límites permitidos.',
                value: coef
            });
        }
    }
    ...
    Object.entries(calculatedValues).forEach(([id, val]) => {
        if (val.total < 0) {
            results.push({
                ...
                message: 'El concepto con ID ' + id + ' tiene un valor negativo (' + val.total.toFixed(2) + ').',
                rowId: id
            });
        }
    });
