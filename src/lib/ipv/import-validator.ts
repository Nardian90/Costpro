...
export class ImportValidator {
  ...
  static normalizeProduct(row: any, rowIndex: number): { product: NormalizedProduct | null, errors: ValidationError[] } {
    ...
    const product: NormalizedProduct = {
      ...
      precio_cents: typeof this.getHeader(row, 'precio_cents') === 'number' ? this.getHeader(row, 'precio_cents') : parseFloat(String(this.getHeader(row, 'precio_cents') || 0).replace(',', '.')),
      costo_unitario_cents: typeof this.getHeader(row, 'costo') === 'number' ? this.getHeader(row, 'costo') : parseFloat(String(this.getHeader(row, 'costo') || 0).replace(',', '.')),
