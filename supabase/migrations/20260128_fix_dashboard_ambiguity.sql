-- Migration: Fix ambiguity in get_dashboard_kpis
-- Date: 2026-01-28

BEGIN;

-- Drop the old version of the function to avoid ambiguity with the new one
-- that includes date range parameters.
DROP FUNCTION IF EXISTS public.get_dashboard_kpis(uuid);

-- Ensure the 3-parameter version exists and is correct (it should have been created by the previous migration)
-- But we recreate it here just to be absolutely sure it's the one we want.
