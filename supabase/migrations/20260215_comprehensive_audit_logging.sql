...
BEGIN
    IF (TG_OP = 'INSERT') THEN
        ...
        VALUES (
            ...
            'products',
            NEW.id,
            jsonb_build_object('name', NEW.name, 'sku', NEW.sku, 'sale_price', NEW.sale_price, 'cost_price', NEW.cost_price),
            NEW.store_id
        );
    ELSIF (TG_OP = 'UPDATE') THEN
        -- Log Price Change
        IF (OLD.sale_price IS DISTINCT FROM NEW.sale_price OR OLD.cost_price IS DISTINCT FROM NEW.cost_price) THEN
            INSERT INTO public.audit_logs (user_id, action, table_name, record_id, old_data, new_data, store_id)
            VALUES (
                auth.uid(),
                'UPDATE_PRICES',
                'products',
                NEW.id,
                jsonb_build_object('sale_price', OLD.sale_price, 'cost_price', OLD.cost_price, 'name', OLD.name),
                jsonb_build_object('sale_price', NEW.sale_price, 'cost_price', NEW.cost_price, 'name', NEW.name),
                NEW.store_id
            );
        END IF;
        ...
        IF (OLD.name IS DISTINCT FROM NEW.name OR OLD.sku IS DISTINCT FROM NEW.sku) THEN
             IF (OLD.sale_price IS NOT DISTINCT FROM NEW.sale_price AND OLD.cost_price IS NOT DISTINCT FROM NEW.cost_price) THEN
                INSERT INTO public.audit_logs (user_id, action, table_name, record_id, old_data, new_data, store_id)
                VALUES (
                    auth.uid(),
                    'UPDATE_PRODUCT',
