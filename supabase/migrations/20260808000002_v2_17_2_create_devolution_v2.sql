-- ============================================================================
-- Migration: 20260808000002_v2_17_2_create_devolution_v2.sql
-- Iteración 11.3 — Fix C-9 + H-Dev1
-- ============================================================================
-- create_devolution_v2: register_stock_movement + idempotency + cross-store
-- validation + audit_logs.
-- ============================================================================

-- ─── UP ──────────────────────────────────────────────────────────────────────

-- 1. Añadir idempotency_key a devolutions
ALTER TABLE public.devolutions ADD COLUMN IF NOT EXISTS idempotency_key TEXT;
CREATE UNIQUE INDEX IF NOT EXISTS devolutions_idempotency_key_idx
  ON public.devolutions (idempotency_key) WHERE idempotency_key IS NOT NULL;

-- 2. Crear create_devolution_v2
...
BEGIN
  ...
  -- 3. Cross-store validation (H-Dev1)
  ...
  -- 6. FOR each item: devolution_items + register_stock_movement + kardex
...
COMMENT ON FUNCTION public.create_devolution_v2 IS
  'Iteración 11.3 (C-9 + H-Dev1): Creates devolution with register_stock_movement, idempotency, cross-store validation, audit_logs.';

-- ─── DOWN ────────────────────────────────────────────────────────────────────
-- DROP FUNCTION IF EXISTS public.create_devolution_v2;
-- DROP INDEX IF EXISTS public.devolutions_idempotency_key_idx;
-- ALTER TABLE public.devolutions DROP COLUMN IF EXISTS idempotency_key;
-- ============================================================================
