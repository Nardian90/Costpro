-- ============================================================================
-- PR-4.4I v2.2.2-R7.2.1 — Modificar create_sale_v2 para INSERT payment_transactions
--
-- Añade INSERTs a payment_transactions al final del RPC, dentro de la misma
-- transacción. Si algo falla, ROLLBACK completo.
-- ============================================================================

-- Recrear create_sale_v2 con INSERTs a payment_transactions
-- Nota: mantenemos la misma signature para no romper callers existentes
-- p_sale_currency y p_sale_exchange_rate se interpretan como la currency/rate
-- del componente Zelle (no de toda la venta)
...
COMMENT ON FUNCTION public.create_sale_v2 IS
  'PR-4.4I v2.2.2-R7.2.1: Server-side checkout with payment_transactions INSERT.
   Cada pago se persiste individualmente con currency + exchange_rate.
   Trigger valida I1a (SUM <= total), I9-TXN (una tasa por método+currency).
   POST-INSERT valida I1b (SUM = total for POS).
   SQLSTATE PT009 (Zelle requires rate), PT011 (payment invariant).';
