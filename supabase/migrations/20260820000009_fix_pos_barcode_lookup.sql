-- ============================================================================
-- Migration: 20260820000009_fix_pos_barcode_lookup.sql
-- FIX: get_products_for_pos ahora busca por barcode exacto
--
-- Antes: el POS solo buscaba por name ILIKE y sku ILIKE
-- Después: también busca por COALESCE(p.barcode, '') = p_search_term
--
-- Esto cierra el circuito: barcode almacenado → POS lo encuentra
-- ============================================================================
...
BEGIN
  ...
  WHERE (p_store_id IS NULL OR p.store_id = p_store_id)
    ...
    AND (
      ...
      OR COALESCE(p.barcode, '') = p_search_term
    )
...
COMMENT ON FUNCTION public.get_products_for_pos IS
  'FIX 20260820000009: Barcode lookup exacto añadido al WHERE clause.
   Busca por: name ILIKE, sku ILIKE, barcode = (exacto).
   Esto permite que scanners físicos encuentren productos por barcode interno.';
