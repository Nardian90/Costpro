-- ============================================================================
-- REM-F4-04 — F4-04 canonical WAC path remediation (gate 20260908-rem-f4-04)
-- ============================================================================
-- DEFECT:  POST /api/inventory/receptions -> register_reception incremented
--          stock but never updated products.cost_average (WAC stayed 0).
-- ROOT:    Body delegated WAC to trg_update_product_wac (absent in live DB)
--          and to register_stock_movement (writes no WAC since A2 v2.22.0).
-- FIX:     register_reception now invokes fn_recalc_wac (single canonical
--          writer, guarded by trg_guard_wac_writer) BEFORE the stock movement,
--          mirroring confirm_pending_reception doctrine (W62-01 §6).
-- ROLLBACK: evidence/rollback_register_reception.sql (prior body preserved in
--           evidence/register_reception_live.sql).
-- No other function, trigger, grant or column is modified by this migration.
-- Signature, defaults, SECURITY DEFINER and search_path are unchanged.
-- ============================================================================
