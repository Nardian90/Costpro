-- =====================================================================
-- REM-INV-4A-R — RC-1 + RC-2 (baseline 751a5c01)
-- fix(security): close supervisor authorization bypass and audit_logs ACL
--
-- RC-1 (F-04-A2, P1): create_sale_v2 supervisor gate.
--   Before: p_supervisor_user_id (client-controlled) reached
--           has_store_role_as() and satisfied the >=15% discount supervisor
--           requirement with ANY admin/manager UUID (cross-tenant included)
--           without supervisor credentials. Reproduced in staging (D4/D5).
--   After:  under authenticated, the ONLY server-verifiable supervisor
--           identity is the caller itself (auth.uid()); foreign identities
--           require the trusted server route (auth.role()='service_role'),
--           which enforces the credential proof (supervisor-check -> signed
--           token -> checkout route) before invoking this RPC.
--   Headers preserved verbatim: LANGUAGE plpgsql / SECURITY DEFINER /
--   SET search_path TO 'public', 'pg_temp' / signature (21 args) / proacl.
--
-- RC-2 (F-04-B, P1-latent): least-privilege ACL on public.audit_logs.
--   Before: anon/authenticated held arwdDxtm (incl. DELETE, TRUNCATE,
--           MAINTAIN). RLS does NOT cover TRUNCATE -> authenticated could
--           destroy the audit trail via direct SQL (G7 staging proof).
--   After:  REVOKE DELETE, UPDATE, TRUNCATE, MAINTAIN from anon and
--           authenticated. SELECT/INSERT preserved (writers architecture);
--           service_role/postgres untouched; RLS and policies untouched.
-- =====================================================================

-- RC-1 (REM-INV-4A-R): create_sale_v2 supervisor authorization gate
-- Surgery: ONLY §11 modified; header/proconfig/proacl untouched.
...
-- RC-2 (REM-INV-4A-R): least-privilege ACL on public.audit_logs
-- Revoke destructive privileges from untrusted client roles.
-- Preserved: SELECT, INSERT (writers architecture), service_role/postgres untouched.
