-- ============================================================================
-- Migration: 20260926000001_esec_price_integrity.sql
-- FASE E-SEC — R-SEC-1: integridad, flexibilidad y autorización del precio de venta
--
-- HALLAZGO (FASE D doc 10 / demostrado en FASE E-SEC pre-fix):
--   create_sale_v2 aceptaba price_at_sale del cliente sin compararlo con el
--   precio de referencia del servidor. El gate de supervisor solo evaluaba el
--   descuento GLOBAL (p_discount_value); el desvío por ítem no generaba
--   descuento global, así que el gate se eludía. ERR_TOTAL_MISMATCH es
--   auto-referencial (recalcula con el MISMO precio del cliente). Vía RPC
--   directo (EXECUTE a authenticated) el route/Zod ni siquiera participa.
--   Demostración pre-fix: 500→1 = 200, 500→300 (40%) sin supervisor = 200,
--   venta $0 = 200, encargado sin autorización 40% = 200.
--
-- CORRECCIÓN (mínima, sin tocar el modelo de datos):
--   1. ERR_INVALID_PRICE: NaN / ±Infinity / negativos rechazados server-side.
--   2. Precio de referencia server-side: product_variants.price (modalidades)
--      o products.price; el desvío agregado por ítem se trata como descuento
--      comercial y hereda la política EXISTENTE (>=15% requiere supervisor
--      admin/manager vía el gate RC-1). No se inventa límite nuevo.
--   3. audit_logs.metadata registra catalog_subtotal / item_discount_total /
--      item_discount_pct (infraestructura de auditoría existente, sin columnas
--      nuevas ni sistemas paralelos).
--
-- INVARIANTE PRESERVADO (objetivo real de la fase): el precio editable es
-- legítimo — 500→490 (2%) sigue creando la venta sin autorización; 500→600
-- (sobrecarga) sigue permitido; sin price en el payload se usa el catálogo.
-- ============================================================================
...
BEGIN
  ...
  -- Se aplica la política EXISTENTE (umbral >=15% requiere supervisor admin/
  -- manager de la tienda, RC-1: identidad supervisor verificable por servidor).
  -- NO se introduce un límite nuevo ni se restringe el precio editable legítimo:
  -- desvíos <15% siguen aceptándose sin autorización.
  IF v_catalog_subtotal > 0 THEN
    v_item_discount_pct := (v_item_discount_total / v_catalog_subtotal) * 100;
  END IF;

  IF v_effective_discount_pct >= 15 OR v_item_discount_pct >= 15 THEN
    IF p_supervisor_user_id IS NULL THEN
      RAISE EXCEPTION 'ERR_SUPERVISOR_REQUIRED: global_pct=%, item_pct=%', v_effective_discount_pct, v_item_discount_pct;
    END IF;
